...
namespace ConnectSphere.Application.Features.Auth.Commands.ReSendEmailVerificationEmail;

public sealed class
    AuthReSendEmailVerificationEmailCommandHandler : IRequestHandler<AuthReSendEmailVerificationEmailCommand,
    ResponseDto<string>>
{
    ...
    public async Task<ResponseDto<string>> Handle(AuthReSendEmailVerificationEmailCommand request,
        CancellationToken cancellationToken)
    {
        var response = await _identityService.CreateEmailTokenAsync(new IdentityCreateEmailTokenRequest(request.Email),
            cancellationToken);
